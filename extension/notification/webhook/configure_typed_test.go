...
func TestConfigureTypedConfigEnabledDisabled(t *testing.T) {
	...
	enabled, err := s.Configure(&notification.Config{Application: config.Config{Notifications: config.NotificationConfig{Webhook: config.WebhookConfig{Endpoint: endpoint}}}})
	...
	enabled, err = (&sender{}).Configure(&notification.Config{Application: config.Config{Notifications: config.NotificationConfig{Webhook: config.WebhookConfig{Endpoint: "://bad"}}}})
	if err == nil || enabled {
