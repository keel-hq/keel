...
func TestConfigureTypedConfigEnabledDisabled(t *testing.T) {
	...
	enabled, err := s.Configure(&notification.Config{Application: config.Config{Notifications: config.NotificationConfig{Discord: config.DiscordConfig{WebhookURL: endpoint}}}})
	...
	enabled, err = (&sender{}).Configure(&notification.Config{Application: config.Config{Notifications: config.NotificationConfig{Discord: config.DiscordConfig{WebhookURL: "://bad"}}}})
	if err == nil || enabled {
