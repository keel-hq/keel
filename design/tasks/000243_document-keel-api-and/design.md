...
| POST | `/v1/webhooks/github` | `receiveGitHubWebhook` / `githubHandler` | Always | Optional `X-GitHub-Event` header selects exact `GitHubPackageV2Webhook` (`package`) or `GitHubRegistryPackageWebhook` (`registry_package`) JSON; absent/other values currently submit an empty event and return 200 | 200 empty; 400 text | Conditional webhook auth |
...
- Swag v1.16.6 represents alternative security schemes with separate `@Security BasicAuth` and `@Security BearerAuth` lines. A single line joined by `||` produces one combined (AND) security object, so annotations use separate lines.
- Source verification corrected the GitHub header assumption: `X-GitHub-Event` is optional at runtime. Only `package` and `registry_package` select payload decoding; an absent or unknown value still submits the handler's empty event and returns 200.
...
- Swag's security-definition parser consumes following security attributes, so global `@tag.name` declarations must precede the security definitions. Spectral then validates with no findings; host/scheme rules are explicitly disabled because the deployment-neutral spec intentionally omits both.
- A generate-twice checksum comparison confirmed deterministic Swagger/client output. CI runs generation, Spectral, the verbose 25-operation report, and `git diff --exit-code` through `make api-check`.
- Final verification passed focused route, conditional-security, and OpenAPI contract tests; Spectral 6.16.1 reported no errors; clean regeneration with swag v1.16.6 and OpenAPI Generator v7.22.0 produced no Git diff; the generated-client Jest test passed under Node 16.20; and the existing UI lint and production build completed. UI checks retain pre-existing warnings for `MiniBar.vue`, Browserslist data, and bundle-size thresholds.
- `go test ./... -timeout=2m` cannot complete in this workspace: `CGO_ENABLED=0` and no C compiler is installed, so existing SQLite-backed tests repeatedly report `go-sqlite3 requires cgo to work` and the suite times out. The new focused `pkg/http` tests are SQLite-independent and pass.
- The final diff was reviewed after merging the latest `origin/master`. It changes documentation comments/models, behavior-preserving route wrappers, generated/docs/tooling files, tests, and CI only; it adds no served documentation endpoint, deployment host, credentials, frontend migration, or API behavior change.
