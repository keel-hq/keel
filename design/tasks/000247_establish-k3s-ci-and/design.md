...
- Docker builds the checked-out Dockerfile, then the script imports the archive into the task-owned containerd and pushes it with containerd's per-command `--plain-http` option. The suite receives the registry-resolved Keel digest through a run-owned environment file.
- The Spec Task sandbox does not provide `make`; script and Go checks can run directly here, while the GitHub runner/local prerequisite documentation retains `make e2e` and `make test` as the public commands.
- The sandbox also has `CGO_ENABLED=0` and no C compiler. Running the direct `make test` equivalent reaches existing SQLite-backed packages but fails during their startup because `go-sqlite3` is a stub without cgo (`sql store startup deadline exceeded`). This is an execution-environment limitation, not a failure in the E2E-only diff; GitHub's Ubuntu runner must run the unchanged `make test` job with cgo available.
- Automatic PR/`master` execution is guarded by the repository variable `KEEL_E2E_PR_GATE=true` pending Nessie's cost approval; `workflow_dispatch` remains directly runnable. A skipped e2e job preserves existing image builds, while an enabled failing e2e job blocks them.
- Clean-cluster verification exposed a production-image defect: Keel's image runs as UID 666, but Alpine does not contain `/run/secrets/kubernetes.io`; kubelet creates the missing projected-volume parent directories as root-only, so both Keel and a minimal UID-666 probe fail to read an otherwise mode-0644 ServiceAccount token. A root probe reads the same token successfully. The narrow fix is to create only those parent directories as mode 0755 during the image build; Keel remains non-root and the token mount remains read-only.
- Custom and standard kubelet state roots showed the same token failure, proving the issue was the image path rather than task-owned k3s storage. The standard kubelet path remains guarded, explicitly unmounted, and deleted only when this run owns the cluster.
- The webhook path can race a newly created Deployment's informer/resource version. The driver now resends the same idempotent registry event within a bounded window until it observes the image transition; this avoids fixed sleeps and does not change Keel production retry behavior.
- Auxiliary `k3s kubectl` and `k3s ctr` commands must receive explicit run-owned `K3S_DATA_DIR` values. Without them, the multicall binary creates default root or home asset caches even when the server itself has an isolated data directory.
- Full smoke runs passed repeatedly at 5m03s cold, 3m59s cached, and 5m01s after client-cache isolation. No scenario flaked after the bounded webhook and final-observation fixes. The measured path is below the hard 10-minute target.
- `make test` passed in a disposable `golang:1.26.5-alpine` container with cgo/build tools because the sandbox host lacks `make` and a compiler. Focused helpers passed repeatedly on the host's Go 1.26.5; `bash -n`, ShellCheck 0.10.0, and actionlint 1.7.7 passed.
- Failed development runs verified that diagnostics are captured before teardown and include Go output, registry/Keel logs, node/pod/resource state, describes, sorted events, and k3s logs. Native k3s writes containerd output into the server stream here, so collection extracts containerd/shim/snapshotter lines into a dedicated `containerd.log` when no standalone file exists.
- No library versions were changed. In particular, Kubernetes/client-go upgrades remain separate work unless this focused stable-API compatibility test demonstrates a concrete failure.
- The required final merge brought in `master`'s separately approved Go 1.26.5 runtime upgrade. The e2e workflow and prerequisite text were aligned to that inherited runtime; this feature still has no dependency-version diff against current `master`.
- The final merged-tree native-k3s run built the Go 1.26.5 Keel image and passed all three scenarios in 5m00s. Its cleanup audit left all guarded runtime/data/kubelet/run paths absent and produced the dedicated containerd diagnostics artifact.
