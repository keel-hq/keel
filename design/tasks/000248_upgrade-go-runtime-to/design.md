...
- Validation passed for the normal `./cmd/keel` binary build, the focused 25-operation API contract test, pinned swag v1.16.6 regeneration, OpenAPI Generator v7.22.0 regeneration, Spectral 6.16.1 lint, and the generated-artifact clean-diff check.
- Full `go test ./... -timeout=5m` compiled under Go 1.26.5 but timed out in existing SQLite tests with `CGO_ENABLED=0`. Retrying with `CGO_ENABLED=1` failed immediately because the configured `gcc` executable is not installed. GNU Make and Yarn are also absent, so the pinned commands underlying `make api-check` were run directly. Integration/e2e execution is unavailable because `kubectl cluster-info` cannot reach a cluster at `localhost:8080`.
- The repository-wide formatting check lists eight pre-existing unformatted Go files outside this change. `go vet ./...` reports pre-existing unkeyed Kubernetes struct literals and unexported JSON-tagged fields, also outside the changed files; neither unrelated issue is modified for this runtime upgrade.
- Container verification passed the default Buildx image for both `linux/amd64` and `linux/arm64` using `golang:1.26.5-alpine`; the ARM64 static CGO layer completed in 674 seconds. Its Node 16/Yarn lint and production-build stages passed. Native AMD64 builds of `Dockerfile.debug` and `Dockerfile.tests` also passed with `golang:1.26.5`.
- `Dockerfile.debian` resolved and entered the Go 1.26.5 builder, but its unchanged `debian:latest` runtime stage failed with `/bin/sh: 1: addgroup: not found`. This predates and is independent of the Go image declaration, so changing the runtime package/user setup would be unrelated scope and is not included.
- Final scope review shows exactly eight changed files with ten one-line replacements: one module directive, two CI pins, five Docker stage pins, and two documentation prerequisites. There are no source, dependency, `go.sum`, generated-artifact, or unrelated changes; `git diff --check` passes.
- The feature branch was fetched against `origin/master` immediately before push and was already up to date. Its sole commit is `0dd48daf` (`build(go): upgrade toolchain to 1.26.5`); no merge commit, release, image push, deployment, publication, or other repository mutation was performed.
