...
- Node metadata is exposed through the existing Kubernetes implementer and cached for 30 seconds by the shared platform resolver. The default chart ClusterRole adds only Node `get`, `list`, and `watch`; no write permission is required.
- `GenericResource` exposes one pod-template/selector abstraction for Deployment, StatefulSet, DaemonSet, and CronJob. The resolver filters nodes using node name, exact selectors, pod OS, and required affinity, then unions platforms observed from matching scheduled pods. Unsupported scheduler constraints are retained as a wider candidate-node set rather than guessed away.
- `cmd/keel` constructs one platform resolver and injects it into both providers. Helm maps a tracked release image to cached resources carrying matching `meta.helm.sh/release-name` and `meta.helm.sh/release-namespace` ownership annotations and the same image repository, then unions their resolved platform sets. Missing mapping fails closed.
- Polling events carry the resolved candidate platform set and a verification marker. Kubernetes and Helm re-resolve current workload eligibility before creating an update plan and reject the event if scheduling metadata changed or the carried manifest set no longer covers the workload. External events omit the marker and retain existing semantics.
- The deterministic integration test runs an in-process OCI registry and drives its tag/config/index metadata through the real registry client, poll watcher, Kubernetes tracked-image resolution, update-plan check, and resource update. It proves `latest` selects `10.10.7` after rejecting `20240303.2-unstable-armhf` on amd64, and proves a mixed amd64/arm64 Deployment selects `2.0.0` after rejecting amd64-only `3.0.0`.
- Focused polling tests capture structured log entries and assert the observable warning branches for an incompatible candidate, unresolved manifest/config metadata, and unresolved workload platform metadata.
- The native k3s e2e harness defines its own ClusterRole instead of consuming chart RBAC. Its first run exposed missing Node reads: policy-negative polling passed by failing closed, but the compatible polling update timed out. Adding the same minimal Node `get/list/watch` rule used by the chart restored meaningful coverage; the rerun passed negative polling, compatible polling, and webhook scenarios.
- Host-wide tests cannot execute SQLite with the sandbox's forced `CGO_ENABLED=0`, but `go test ./...` passes in a cgo-enabled Alpine container. Focused tests, compile-only `go test ./... -run '^$'`, `go build ./cmd/keel`, OpenAPI contract tests, the Docker cgo/static build, UI checks in that build, and the native k3s e2e suite pass. Repository-wide `go vet` still reports the pre-existing unkeyed Kubernetes literals and JSON tags on unexported `util/image.Reference` fields.
- Final review added explicit Kubernetes and Helm update-plan tests for the second compatibility gate: a verified event carrying an incompatible platform set is rejected after current workload platforms are re-resolved, while the same policy/tag with compatible evidence still yields one plan. No workload compatibility path references `runtime.GOOS`/`runtime.GOARCH` or parses architecture from tag names; Node RBAC is read-only and no dependency or UI source changed.
- PR #873's initial CI run failed only in `Check generated API artifacts`: Swag interpreted the internal `Repository.Platforms` and `Repository.PlatformVerified` polling evidence as public API fields, causing `docs/swagger.yaml` and the generated JavaScript client to drift. The fields must retain JSON serialization so approval persistence preserves compatibility evidence, so the focused fix adds `swaggerignore:"true"` rather than removing their JSON representation. Re-running the exact Swagger and OpenAPI client generators produced no tracked diff; focused platform/update tests, the OpenAPI contract test, and repository-wide compile checks also pass.
- The PR k3s job was skipped by an intentional job-level opt-in (`workflow_dispatch` or repository variable `KEEL_E2E_PR_GATE=true`), while the Docker job explicitly accepted that skipped result. User approval to make k3s part of normal CI removed that condition, so the existing `pull_request`, master/tag push, and manual triggers all execute `make e2e` and Docker still waits for its result.
- Exercising the newly enabled job locally exposed a harness-only false failure: the shared `client-go` client used its 5 QPS/10 burst defaults while helpers poll every 250 ms, and a final StatefulSet read was rejected by the client-side limiter near the assertion deadline. The isolated test client now uses 50 QPS/100 burst; assertions and observation windows are unchanged. The immediate native-k3s rerun passed all four scenarios in about 220 seconds, within the CI job's 12-minute timeout.
