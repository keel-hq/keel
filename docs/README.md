...
`openapi-generator-config.yaml` and `openapi-generator-ignore` configure OpenAPI Generator v7.22.0. `make api-client` replaces `ui/src/api/generated` with the ES-module contract client generated from `swagger.yaml`; it is retained as a checked contract artifact and covered by the frontend tests. The React runtime integration lives in `ui/src/lib/api.ts` so authentication and 401 handling remain explicit.
